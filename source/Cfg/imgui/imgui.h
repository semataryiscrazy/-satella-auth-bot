...
namespace ImGui
{
    ...
    IMGUI_API ImU32         GetColorU322(const ImVec4& col, float alpha_mul = 1.f);
    ...
    IMGUI_API bool          TabDP(const char* icon, bool selected);
    IMGUI_API bool          Tab(const char* Label, const char* Icon, bool Enabled);
    IMGUI_API bool          TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs, int TabIndex);
    bool TTab(const char* icon, const char* label, bool active);
    // button
